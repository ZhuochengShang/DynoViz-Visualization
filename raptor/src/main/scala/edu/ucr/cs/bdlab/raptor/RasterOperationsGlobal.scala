...
object RasterOperationsGlobal {
  ...
  def rasterizePoints[T: ClassTag](points: RDD[(Double, Double, T)], metadata: RasterMetadata, rasterFeature: RasterFeature): RasterRDD[T] = {
    rasterizePixels(points.map(p => {
      ...
      //println(p._1, p._2, Math.round(geolocation(0)).toInt, Math.round(geolocation(1)).toInt, p._3)
      //(Math.round(geolocation(0)).toInt, Math.round(geolocation(1)).toInt, p._3)
